...
- [ ] 1.11 **BLOCKED** — Apply to test Supabase project (`grnamollopxdlstcpxhc`) via MCP; confirm with `list_tables`/`get_advisors`; fix any advisories. Neither the Supabase MCP tool nor the Supabase CLI (`db push` requires `--password`/`--db-url`, and no DB password/access token is present in `.env`/`.env.local`/shell env, only the REST API URL + anon/service keys) was available/usable in this `sdd-apply` execution context. The migration SQL is authored, locally reviewed (balanced parens, cross-checked against live OpenAPI schema and 0008 precedent) and committed, but **not yet applied to the live test database**. A follow-up `sdd-apply` run with Supabase MCP tool access (or the DB password) must complete this task — apply, run `get_advisors`, and fix any advisories (e.g. mutable `search_path`) — before Phase 2 (`0012_pcp_extras.sql`) can build on top of this schema.
...
- [ ] 2.1 Verify live schema via Supabase MCP `list_tables`/`execute_sql` for `precios_proveedor`, `terceros`, `terceros_contactos`, `condiciones_pago`, `formas_pago`, `sectores_contacto`, `usuarios`, `costos_productos`, `v_precios_especiales_vigentes`, `v_presupuesto_revision` — do NOT trust `docs/schema/extractor_final.sql` (read-only, known stale).
- [ ] 2.2 Create `supabase/migrations/0012_pcp_extras.sql`; version guard consistent with `0011_pcp_modelo.sql`'s M0, applied after PR1 merges.
- [ ] 2.3 M1: `pcp_historial` table (D6) — append-only, no `updated_at`, index `(drogueria_id, pcp_id, created_at DESC)`.
- [ ] 2.4 M2: `reglas_pcp` table (D7) — seam only, no rows/service code.
- [ ] 2.5 M3: `pcp_legacy_map` table (D8) — `UNIQUE (drogueria_id, sistema_origen, codigo_legacy)`.
- [ ] 2.6 M4: `pcp_consultas` + `pcp_consulta_renglones` tables (D9) — no `pcp_id` on `pcp_consultas`.
- [ ] 2.7 M5: `ALTER TABLE precios_proveedor ADD condicion_pago_id UUID NULL, ADD forma_pago_id UUID NULL` + composite FKs (D5).
- [ ] 2.8 M5b: backfill — per `drogueria_id`, find-or-create `condiciones_pago` row for each distinct `plazo_pago_dias`, set the FK; keep `plazo_pago_dias` nullable/unused.
- [ ] 2.9 M6: DROP+CREATE `v_precios_especiales_vigentes` and `v_presupuesto_revision` reading `COALESCE(cp_pp.plazos_dias[1], pp.plazo_pago_dias, cp_prov.plazos_dias[1])`, preserving `WITH (security_invoker = true)` on both — this is the exact class of mistake migration 0008 documented making once; verify with `pg_get_viewdef` after apply, not just by reading the migration source.
- [ ] 2.10 M7: RLS `ENABLE` + policies per D11 on `pcp_historial`, `reglas_pcp`, `pcp_legacy_map`, `pcp_consultas`, `pcp_consulta_renglones` (SELECT: `superadmin,admin,gerencia,compras`; INSERT/UPDATE: `admin,gerencia,compras`; DELETE: `es_superadmin()`); `pcp_historial` gets SELECT+INSERT only; `GRANT`s; `trg_set_updated_at` where applicable; `NOTIFY pgrst, 'reload schema'`.
- [ ] 2.11 M8: `upsert_pcp_legacy` RPC (D8) — no `SECURITY DEFINER`, `SET search_path = public, pg_temp`, `REVOKE EXECUTE` from `PUBLIC`/`anon`/`authenticated`, `GRANT` to `service_role`.
- [ ] 2.12 Create `supabase/migrations/0012_pcp_extras.down.sql` mirroring M1-M8 in reverse.
- [ ] 2.13 Update `docs/schema/extractor_final.sql` and `docs/schema/rls_final.sql` snapshots for these five tables, the `precios_proveedor` FK columns, and the two recreated views.
- [ ] 2.14 Apply to test Supabase project (`grnamollopxdlstcpxhc`) via MCP; confirm with `list_tables`/`get_advisors`; fix any advisories.
- [ ] 2.15 Integration test: `plazo_pago_dias` backfill produces one `condiciones_pago` row per distinct value per `drogueria_id`; both recreated views still resolve against seeded data.
