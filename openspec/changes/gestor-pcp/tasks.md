...
- [ ] 2.14 Apply to test Supabase project (`grnamollopxdlstcpxhc`) via MCP; confirm with `list_tables`/`get_advisors`; fix any advisories. **BLOCKED**: `mcp__supabase__*` tools were not present in this `sdd-apply` execution context's tool list (confirmed before starting, per the orchestrator's instruction) — same situation as PR1's 1.11. Per explicit orchestrator instruction, this is left undone rather than silently skipped or worked around via CLI/psql; the orchestrator will apply `0012_pcp_extras.sql` directly via MCP, exactly as it did for `0011_pcp_modelo.sql`. Everything downstream of a live apply (`pg_get_viewdef` check for 2.9, `get_advisors`, a real GREEN run of 2.15) is pending on this.
- [x] 2.15 Integration test: `plazo_pago_dias` backfill produces one `condiciones_pago` row per distinct value per `drogueria_id`; both recreated views still resolve against seeded data. Written as `tests/pcp/test_backfill_plazo_pago.py` (4 tests: distinct-value backfill + idempotency, tenant isolation, `v_precios_especiales_vigentes` resolves with the backfilled value, `v_presupuesto_revision` resolves without error) calling the named `backfill_condicion_pago_desde_plazo` RPC from 2.8. **Confirmed RED for the expected reason**: 3/4 fail with `PGRST202 Could not find the function public.backfill_condicion_pago_desde_plazo` (the function doesn't exist live yet — blocked by 2.14); the 4th (`v_presupuesto_revision` resolves, pre-existing view shape) passes. Will need a re-run once 2.14 unblocks to confirm real GREEN. **Also discovered while writing this test**: the shared root fixture `tests/conftest.py::seed_proveedor` is broken repo-wide — it inserts `razon_social` directly into `proveedores`, a column migration `0008_terceros_modelo.sql` moved to `terceros` months ago (confirmed the same `PGRST204` failure reproduces against an existing, unrelated `tests/pricing/test_service.py` test). Pre-existing bug, not introduced by gestor-pcp and out of this PR's scope to fix repo-wide; worked around locally with a corrected `tests/pcp/conftest.py::seed_proveedor_pcp` fixture (two-step terceros+proveedores insert) instead of touching the shared fixture. Flagged here for the orchestrator/a follow-up change to fix `tests/conftest.py` itself.
