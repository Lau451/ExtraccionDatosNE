...
`pcp_legacy_map`: `pcp_id`, `sistema_origen` default `'legacy'`, `codigo_legacy`, `datos_legacy JSONB`, `importado_at`; idempotency key `UNIQUE (drogueria_id, sistema_origen, codigo_legacy)`. Unlike `terceros_legacy_map` there is no `entidad_legacy` discriminator — PCP is a single entity, so the code spaces cannot collide.

**Confirmed export contract (13 columns, row-per-renglón, denormalized)**: the legacy system has no presupuesto concept, so it exports one flat row per renglón, with header fields repeating across every row that shares the same `número de PCP`. Confirmed columns: código del cliente, razón social del cliente, número de PCP (unique per PCP; anchors `pcp_legacy_map.codigo_legacy`), **número de presupuesto** (optional, archival-only — see below), **proceso comercial** (optional, `1`=cotización directa / `2`=licitación, defaults when absent — see below), importe total, fecha de generación, fecha de respuesta esperada, renglón (matches `items_proceso.numero_renglon`), código del producto, descripción del producto, cantidad del producto, precio del producto.

**Resolution of the design's original hard blocker.** The first draft assumed `items_proceso` rows already existed and legacy import only needed to *match* `pcp_renglones.item_proceso_id` against them via a renglón-level code. That assumption does not hold for this dataset: `pcp.presupuesto_id` and `pcp.proceso_comercial_id` are `NOT NULL` FKs to real UUID rows in `presupuestos`/`procesos_comerciales` (`fk_pcp_presupuesto`, `fk_pcp_proceso` in `0011_pcp_modelo.sql`), and `pcp_renglones.item_proceso_id` is a `NOT NULL` FK to `items_proceso`, which itself requires `proceso_comercial_id NOT NULL` with `UNIQUE (proceso_comercial_id, numero_renglon)` (`docs/schema/extractor_final.sql`). Since the old system has no presupuestador, **no `procesos_comerciales`/`presupuestos`/`items_proceso` rows exist yet for these legacy PCPs** — `upsert_pcp_legacy` must **find-or-create** all three, not merely match against pre-existing data. The user explicitly rejected relaxing these `NOT NULL` constraints (which would also touch `uq_pcp_presupuesto_abierto`'s semantics) in favor of generating internal placeholders — the schema stays exactly as shipped in PR1.

Resolved flow, keyed off `pcp_legacy_map` (no additional legacy-map table — an earlier draft of this design proposed a dedicated `presupuestos_legacy_map`; the user rejected that as over-engineering, since `pcp_legacy_map` already anchors everything needed):

- **First import of a given "número de PCP"** (no existing `pcp_legacy_map` row): `cliente_id` resolves via the **already-existing** `terceros.codigo_interno` legacy code path (from "código del cliente" — no new legacy map needed). A **placeholder** `procesos_comerciales` row is created: `clase` from the `1`/`2` "proceso comercial" mapping (`clase CHECK IN ('cotizacion','licitacion')` already exists with exactly these two values — no new enum); **if "proceso comercial" is absent, `clase` defaults to `'licitacion'`** (confirmed by the user — licitación, not cotización, is the fallback). `nombre` (`NOT NULL`) is synthesized (e.g., from cliente + PCP number) since the legacy export carries no process name. A **placeholder** `presupuestos` row is created tied to that `proceso_comercial_id` (`estado='generado'` table default; `monto_total` from "importe total" if present else `NULL`; `cantidad_items` = renglón count for that PCP in the batch). The `pcp` row is created with these freshly generated `presupuesto_id`/`proceso_comercial_id`, `origen='import_legado'`. `pcp_legacy_map` is written with `codigo_legacy` = número de PCP and `datos_legacy` = the raw header JSON — this is where "número de presupuesto", if the old system ever sends one, is archived **purely as informational metadata, never as a lookup/resolution key**.
- **Re-import of the same "número de PCP"** (a `pcp_legacy_map` row already exists): the existing `pcp` row is found via the map, and its **already-stored** `presupuesto_id`/`proceso_comercial_id` are reused as-is. `procesos_comerciales`/`presupuestos` are **never touched again** on a re-import — idempotency comes entirely from the existing `pcp`/`pcp_legacy_map` pair, which task 8.3 already covers ("re-importing the same codigo_legacy updates the existing PCP").
- `items_proceso`: created per renglón row, keyed by `(proceso_comercial_id, numero_renglon)` from the "renglón" column, once the parent `proceso_comercial_id` is resolved (from either branch above).
- Because multiple rows share one header, the header-level resolution (cliente, proceso_comercial, presupuesto) happens once per distinct "número de PCP" (via the existing `pcp_legacy_map` lookup) and is reused for every renglón row sharing it in the batch.

This means "número de presupuesto" is optional export data with no structural role (archived only), and "proceso comercial" is effectively optional too now that it has a resolved default. Once the presupuestador exists, future imports carry a real native `presupuestos.id`/number directly instead of a legacy placeholder.

RPC `upsert_pcp_legacy(p_drogueria_id, p_sistema_origen, p_filas JSONB, p_usuario_id) RETURNS TABLE (codigo_legacy, pcp_id, accion)` mirrors `upsert_terceros_legacy` step for step: `SELECT ... FOR UPDATE` on the map, insert-or-update the header, `INSERT ... ON CONFLICT DO NOTHING` on the map, then renglones with `origen='import_legado'`. `LANGUAGE plpgsql`, `SET search_path = public, pg_temp`, **no** `SECURITY DEFINER` (invoked via `get_service_client()`, which already bypasses RLS), `REVOKE EXECUTE` from `PUBLIC`/`anon`/`authenticated`, `GRANT` to `service_role`. Note: the RPC signature/body above predates the find-or-create scope expansion and will need revisiting during PR8's own `sdd-design` pass to account for the `procesos_comerciales`/`presupuestos`/`items_proceso` writes.
...
- [x] **Legacy PCP export contract.** Confirmed: 13-column, row-per-renglón denormalized export (D8) — código/razón social del cliente, número de PCP, número de presupuesto (optional, archival-only), proceso comercial (optional, 1=cotización directa/2=licitación, defaults to licitación when absent), importe total, ambas fechas, renglón, código/descripción/cantidad/precio del producto. `pcp_renglones.item_proceso_id` no longer resolves via a simple matching step: since the legacy system has no presupuestador, `procesos_comerciales`/`presupuestos`/`items_proceso` rows are **found-or-created** during import. Idempotency sub-question resolved: no dedicated legacy-map table for `procesos_comerciales`/`presupuestos` — `pcp_legacy_map` already anchors re-imports, so placeholders are generated only on first import and reused as-is afterward (D8).
