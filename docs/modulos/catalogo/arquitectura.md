...
Catálogo no importa de ningún otro módulo de negocio de `presupuestacion/` para su
...
`DEPOSITO_SENTINEL = "unico"` está definido en `imports/service.py:18`. Catálogo lo
...
Ver RN-CATALOGO-009 en [`reglas.md`](./reglas.md) y D-CATALOGO-004 en
...
o el módulo se elimina, `catalogo/repository.py` se rompe en tiempo de import.
...
- **Catálogo** (`ajustar_stock`, `service.py:236-250`) escribe únicamente
  `cantidad_disponible`, vía `upsert_stock` (`repository.py:185-191`), con un
  comentario explícito en el código (cita textual verificada en RN-CATALOGO-010,
...
`cantidad_comprometida`. Es decir: la separación de responsabilidad no es "Catálogo
escribe disponible, Core escribe comprometida" en términos absolutos, sino "Catálogo
...
Esto vuelve concreto —no solo teórico— el riesgo señalado en D-CATALOGO-003
([`decisiones.md`](./decisiones.md)): `catalogo.ajustar_stock` hace un `upsert` sin
...
(`core/stock.py:47-54`). Si un ajuste manual de Catálogo y una entrega de OC caen
sobre la misma fila de `stock_productos` en una ventana de tiempo corta, el `upsert`
de Catálogo puede pisar el valor que `core/stock.py` acaba de descontar, porque
Catálogo no relee ni compara nada antes de escribir — el guard de optimistic locking
...
Catálogo, sin pasar por `catalogo/repository.py` ni por `catalogo/service.py`. Es el
...
deleted_at IS NULL`, en paralelo a `catalogo.repository.listar_productos`. Documentado
...
`catalogo.repository.obtener_proveedor`.
...
- `pricing/repository.py:57` (`buscar_costo_estandar_vigente`):
  ...
  misma condición de vigencia que usa `catalogo.repository.costo_vigente`
...
`catalogo.ajustar_stock`. Puntos de acceso a la tabla: `core/stock.py:17`
...
carga masiva, sin relación de código con `catalogo/repository.py`:
...
- `-- productos --` (líneas 5-61): incluye un **upsert masivo**
  ...
  `catalogo.service` (ni `exclude_unset`, ni resolución de `categoria_id`, etc.).
- `-- costos --` (líneas 64-84).
- `-- stock --` (líneas 87-91).
...
algoritmo** que `catalogo.service.crear_costo` (`service.py:195-218`,
RN-CATALOGO-005/006) — cerrar el costo vigente con `fecha_hasta = fecha_desde - 1
...
vez de `origen="manual"` (`service.py:216`, RN-CATALOGO-007). Las dos
...
```
     ...
     ┌──────────┬───────────┼───────────┬──────────────┬─────────────┐
     │          │           │           │              │             │
catalogo/   matching/  comparativas/ pricing/     core/stock.py  imports/
