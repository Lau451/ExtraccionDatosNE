...
| `nombre`, `categoria_id`, `clasificacion`, `droga`, `presentacion`, `forma_farmaceutica`, `marca_id`, `envase_id`, `alicuota_iva`, `codigo_anmat`, `datos_sistema` | Escritas 1:1 desde `ImportProductoRow` en cada alta/actualización (`service.py`), sin ninguna validación adicional (no resuelve si `categoria_id`/`marca_id`/`envase_id` existen, por ejemplo). `laboratorio` se reemplazó por `marca_id` en la migración 0016 — `ImportProductoRow` ya no lo acepta. |
