...
> `proveedores` (`ProveedorCreate`/`Update`/`Out`, `crear_proveedor`,
> `listar_proveedores`, `obtener_proveedor`, `actualizar_proveedor`,
> `eliminar_proveedor` y sus `_para_endpoint`) se **eliminó por completo** — ya no
> existe en este módulo. Las firmas de `proveedores` que siguen abajo describen el
> estado anterior a esa eliminación y no fueron purgadas línea por línea en este
> refactor; para la API vigente de proveedores ver [`../terceros/api.md`](../terceros/api.md).
...
class CostoCreate(BaseModel):
    ...
    fecha_desde: date
# models.py:109-111
# Sin campo origen — este módulo siempre lo hardcodea a "manual" (RN-PRODUCTOS-007).
...
# models.py:123-125
# Sin campo cantidad_comprometida — no es escribible desde este módulo (RN-PRODUCTOS-010).
...
# repository.py:74-75
# Sin función de borrado — RN-PRODUCTOS-004.
...
# deposito=None → usa DEPOSITO_SENTINEL (RN-PRODUCTOS-009).

def upsert_stock(client: Client, fila: dict[str, Any]) -> dict[str, Any]: ...
# repository.py:185-191
# on_conflict="producto_id,deposito" — RN-PRODUCTOS-008.
```
...
# service.py:100-107
# Pertenencia (variante categoría de RN-PRODUCTOS-001) + RN-PRODUCTOS-002.
# Sin eliminar_categoria — RN-PRODUCTOS-004.
...
# service.py:195-218
# RN-PRODUCTOS-001 + RN-PRODUCTOS-005 + RN-PRODUCTOS-006 + RN-PRODUCTOS-007.
...
# service.py:236-250
# RN-PRODUCTOS-001 + RN-PRODUCTOS-008 + RN-PRODUCTOS-009 + RN-PRODUCTOS-010.
# Docstring textual citado en reglas.md RN-PRODUCTOS-010 (service.py:239-240).
...
eliminar — RN-PRODUCTOS-004), 3 de proveedor, 2 de costo y 2 de stock. Ver
[`decisiones.md`](./decisiones.md) D-PRODUCTOS-001.
...
`NotFoundError`→404 (RN-PRODUCTOS-001, actualización de categoría inexistente). No se
encontró ningún `raise` de `ValidationError` ni `ForbiddenError` en este módulo — a
diferencia de [`../clientes/`](../clientes/), Productos usa una sola excepción de
