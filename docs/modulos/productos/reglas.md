...
> `proveedores` se **eliminó por completo**. Las reglas de abajo que mencionan
> `proveedor(es)` (partes de RN-PRODUCTOS-001/002/003) describen comportamiento que
> ya no existe en este módulo — no fueron reescritas para quitar esas menciones en
> este refactor.

Todas las reglas fueron verificadas contra el código real (`service.py`, `router.py`)
y sus tests (`tests/productos/test_service.py`) en esta sesión.

### RN-PRODUCTOS-001 — Un producto o proveedor solo es consultable/modificable si pertenece a la droguería del solicitante
...
- **Prioridad**: Alta.
- **Archivo**: `services/productos/service.py:51-55` (producto),
  `:149-153` (proveedor).
- **Observaciones**: [IMPLEMENTADO]. `obtener_producto` es reutilizada como primer
  ...
  producto en `tests/productos/test_service.py:87-97`
...
### RN-PRODUCTOS-002 — Las actualizaciones de producto, categoría y proveedor son parciales
...
- **Prioridad**: Alta.
- **Archivo**: `services/productos/service.py:62`, `:106`, `:160`.
...
### RN-PRODUCTOS-003 — La baja de producto y proveedor es siempre soft-delete
...
- **Resultado**: `repo.soft_delete_producto(...)` / `repo.soft_delete_proveedor(...)`
  — `UPDATE`, nunca `DELETE`.
- **Prioridad**: Alta.
- **Archivo**: `services/productos/repository.py:46-53` (producto),
  `:114-121` (proveedor); invocadas desde `service.py:69` y `:167`.
- **Observaciones**: [IMPLEMENTADO]. Verificada en
  `tests/productos/test_service.py:123-140`
  (`test_eliminar_producto_soft_delete`, confirma que `obtener_producto` deja de
  encontrarlo tras el borrado) y `tests/productos/test_service.py:177-210`
  (`test_crear_listar_actualizar_y_eliminar_proveedor`, líneas 205-210, mismo patrón
  para proveedor).

### RN-PRODUCTOS-004 — Categorías no tienen baja: ni soft-delete ni endpoint `DELETE`
...
- **Resultado**: no aplica.
- **Prioridad**: Media.
- **Archivo**: `services/productos/repository.py` (búsqueda de
...
- **Observaciones**: [IMPLEMENTADO] la ausencia. Ver D-PRODUCTOS-005 en
  [`decisiones.md`](./decisiones.md).

### RN-PRODUCTOS-005 — Un costo nuevo con el mismo valor que el vigente no genera fila nueva
...
- **Prioridad**: Alta.
- **Archivo**: `services/productos/service.py:201-202`.
- **Observaciones**: [IMPLEMENTADO]. Verificada en
  `tests/productos/test_service.py:245-256`
  (`test_crear_costo_manual_no_duplica_si_es_igual`, llama `crear_costo` dos veces con
  el mismo body y confirma `len(historial) == 1`).

### RN-PRODUCTOS-006 — Un costo con valor distinto versiona: cierra el vigente e inserta uno nuevo
...
- **Resultado**: `repo.cerrar_costo_vigente(client, costo_id=vigente["id"],
  ...
  `fecha_hasta=None`.
- **Prioridad**: Alta.
- **Archivo**: `services/productos/service.py:204-218`.
- **Observaciones**: [IMPLEMENTADO]. Verificada en
  `tests/productos/test_service.py:217-242`
...
### RN-PRODUCTOS-007 — El origen de un costo creado por este módulo es siempre `"manual"`
...
- **Condición**: cualquier llamada exitosa a `crear_costo` que efectivamente inserte
  (RN-PRODUCTOS-006, o primera alta sin vigente previo).
- **Resultado**: `"origen": "manual"`.
- **Prioridad**: Media.
- **Archivo**: `services/productos/service.py:216`.
- **Observaciones**: [IMPLEMENTADO]. Contrasta con `imports/service.py:115`, `:132`,
  ...
  `tests/productos/test_service.py:236`
  (`assert resultado["origen"] == "manual"`).

### RN-PRODUCTOS-008 — El ajuste de stock es un upsert idempotente por depósito
...
- **Prioridad**: Alta.
- **Archivo**: `services/productos/repository.py:185-191`.
- **Observaciones**: [IMPLEMENTADO]. Verificada en
  `tests/productos/test_service.py:281-297`
...
### RN-PRODUCTOS-009 — Sin depósito especificado, se usa el sentinel `"unico"`
...
- **Prioridad**: Media.
- **Archivo**: `services/productos/service.py:247`;
  `services/productos/repository.py:173` (mismo patrón para
  `buscar_stock_por_deposito`).
- **Observaciones**: [IMPLEMENTADO]. Ver D-PRODUCTOS-004 en
  ...
  `tests/productos/test_service.py:281-297` (el resultado queda con
  `deposito == "unico"`, línea 298).

### RN-PRODUCTOS-010 — El ajuste manual de stock nunca modifica `cantidad_comprometida`
...
- **Prioridad**: Alta.
- **Archivo**: `services/productos/service.py:236-250` (comentario en
  `:239-240`, dict del upsert en `:244-249`, sin la clave `cantidad_comprometida`).
- **Observaciones**: [IMPLEMENTADO]. Verificada en
  `tests/productos/test_service.py:263-278`
